---
title: JtChat Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for JtChat Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest JtChat image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running JtChat Container

Assuming the JtChat container is named `lobe-chat`, use the following commands to stop and remove the currently running JtChat container:
...
### Pull the Latest JtChat Image

Use the following command to pull the latest Docker image for JtChat:
...
Redeploy the JtChat container using the newly pulled image:
...
No need to worry, you won't. All of JtChat's chat records are stored in your local browser. Therefore, when redeploying JtChat using Docker, your chat records will not be lost.
