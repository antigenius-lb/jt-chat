---
title: Customizing Provider Model List in JtChat for Deployment
description: >-
  Learn how to customize the model list in JtChat for deployment with the
...
JtChat supports customizing the model list during deployment. You can use `+` to add a model, `-` to hide a model, and use `model name=display name<extension configuration>` to customize the display name of a model, separated by English commas. The basic syntax is as follows:
