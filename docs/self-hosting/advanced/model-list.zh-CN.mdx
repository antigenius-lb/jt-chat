---
title: JtChat 自定义模型服务商模型列表及扩展能力配置
description: 了解如何在 JtChat 中自定义模型列表以及扩展能力配置的基本语法和规则。
tags:
  - JtChat
  - 自定义模型列表
  - 扩展能力配置
...
JtChat 支持在部署时自定义模型列表，可以使用 `+` 增加一个模型，使用 `-` 来隐藏一个模型，使用 `模型名=展示名<扩展配置>` 来自定义模型的展示名，用英文逗号隔开。通过 `<>` 来添加扩展配置。基本语法如下：
