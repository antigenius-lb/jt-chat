---
title: Configuring Github Authentication Service for JtChat
description: >-
  Learn how to configure Github authentication service for JtChat, including
...
| --- | --- | --- |
| `ENABLE_OAUTH_SSO` | Required | Enable Single Sign-On (SSO) for JtChat. Set to `1` to enable SSO. |
...
| `SSO_PROVIDERS` | Required | Select the Single Sign-On provider for JtChat. Use `github` for Github. |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate with Github and use JtChat.
</Callout>
