---
title: Configure Auth0 Identity Verification Service for JtChat
description: >-
  Learn how to configure Auth0 Identity Verification Service for JtChat for
...
Click on the "Users Management" in the left navigation bar to enter the user management interface, where you can create users for your organization to log in to JtChat.
...
When deploying JtChat, you need to configure the following environment variables:

| Environment Variable | Type | Description |
| --- | --- | --- |
| `ENABLE_OAUTH_SSO` | Required | Enable single sign-on (SSO) for JtChat. Set to `1` to enable single sign-on. |
...
<Callout>
  After successful deployment, users will be able to authenticate and use JtChat using the users
  configured in Auth0.
</Callout>
...
<Callout type={'warning'}>
  ...
  JtChat being abused by external users.
</Callout>
