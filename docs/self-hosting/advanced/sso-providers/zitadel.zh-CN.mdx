---
title: 在 JtChat 中配置 ZITADEL 身份验证服务
description: 学习如何在 JtChat 中配置 ZITADEL 身份验证服务，包括创建应用、配置环境变量等步骤。
tags:
  - ZITADEL
  - 身份验证服务
  - 环境变量配置
...
<Callout type={'important'}>
  ...
  - 请将 `http(s)://your-domain` 替换为 JtChat 部署的实际 URL。
</Callout>
...
| `SSO_PROVIDERS` | 可选 | 为 JtChat 选择单点登录提供程序。对于 ZITADEL，请填写 `zitadel`。 |
...
<Callout>部署成功后，用户将能够通过 ZITADEL 中配置的用户进行身份验证并使用 JtChat。</Callout>
