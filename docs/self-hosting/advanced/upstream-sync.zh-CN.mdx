---
title: JtChat 启动自动更新 - Vercel / Zeabur 部署
description: 按照指南重新部署项目以解决 JtChat 在 Vercel 中默认创建新项目导致的“有可用更新”提示问题，并启用自动更新。
tags:
  - Vercel
  - Zeabur
  - 自动更新
...
<Steps>
  ### 停止并删除当前运行的 JtChat 容器

假设 JtChat 容器的名称是 `lobe-chat`，使用以下指令停止并删除当前运行的 JtChat 容器：
...
放心，不会的。JtChat 的聊天记录全部都存储在你的本地浏览器中。因此使用 Docker 重新部署 JtChat 时，你的聊天记录并不会丢失。
