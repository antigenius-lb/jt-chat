---
title: JtChat 支持多模型服务商
description: 了解 JtChat 在多模型服务商支持方面的最新进展，包括已支持的模型服务商和计划中的扩展，以及本地模型支持的使用方式。
tags:
  - JtChat
  - AI 会话服务
  - 模型服务商
...
为了满足特定用户的需求，JtChat 还基于 [Ollama](https://ollama.ai) 支持了本地模型的使用，让用户能够更灵活地使用自己的或第三方的模型，详见 [本地模型支持](/docs/usage/features/local-llm)。
