---
title: Using Local Qwen Model in JtChat
description: 通过JtChat和Ollama的集成，您可以轻松在JtChat中使用Qwen。本文将指导您如何在JtChat中使用本地部署版本的Qwen。
tags:
  - Qwen
  - JtChat
  - Ollama
...
Now, through the integration of JtChat and [Ollama](https://ollama.com/), you can easily use Qwen in JtChat. This document will guide you on how to use the local deployment version of Qwen in JtChat:
...
In the JtChat conversation page, open the model selection panel, and then select the Qwen model.
...
/>
  ...
  <Callout type={'info'}>
    If you do not see the Ollama provider in the model selection panel, please refer to [Integration with Ollama](/docs/self-hosting/examples/ollama) to learn how to enable the Ollama provider in JtChat.

</Callout>
</Steps>

Next, you can have a conversation with the local Qwen model in JtChat.
